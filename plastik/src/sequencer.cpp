...
void Sequencer::executeStepPoly() {
    ...
    for (int i = 0; i < MAXNOTES; i++) {
        ...
        uint32_t bitMask = 3;
        bitMask = bitMask << (i * 2);
        uint32_t currentStep = patterns[pattern].notes[stepPos];
        uint8_t currentNote = (currentStep & bitMask) >> (i * 2);

        if (currentNote) {
            digitalWrite(gates[iOutput], HIGH);

            int pitch = i;
            cvs[iOutput] = currentNote * i * 100;
    ...
    dac.analogWrite(cvs[3], cvs[2], cvs[1], cvs[0]);
 ...
 */
void Sequencer::triggerChangePattern() {
